...
# Make full dataset and block identifier:
...
# Make five versions of the training dataset, where each
# corresponds to one training data missing pattern in Figure 1
# of the paper:
...
perminds <- sample(1:nrow(datatrain))
datatrain2 <- datatrain[perminds,]
...
# Make four versions of the test dataset, where each
# corresponds to one test data missing pattern in Figure 1
# of the paper:
...
save(datatrain1, datatrain2, datatrain3, datatrain4, datatrain5,
     ...
     file="./Code/ExampleData.Rda")




myimage <- function(x)
  image(t(x)[,ncol(t(x)):1])

myimage(is.na(datatrain2))
