...
multisfor <- function(data, folds, num_trees = 500, mtry = NULL, min_node_size = 1) {
  " Train a seperate RF on each fold available in 'data', such that the final RF
    ...
    Return: List with as many fold-wise fitted RFs (of class 'multisourceRF')
...
predict.multisfor <- function(object, data) {
  " Get predicitons for the 'data' from a fold-wise fitted RF.
  ...
  for (i in 1:length(object)) {
    tree_testsets[[i]] <- process_data(tree = object[[i]][[1]], 
                                            data = data)
  }
...
if (FALSE) {
  ...
  save(datatrain, foldstrain, datatest, file="./Data/ExampleData.Rda")
